...
# ITEM'S ABILITIES
      # INVERTOR SHARD
            ...
            execute as @e[type=item,tag=!notCollected,tag=!returnItem,nbt={Item:{tag:{pjItem:["yes","invertorShard"]}}}] if entity @a[scores={ball.own=1..}] at @s as @a[sort=nearest,limit=1] unless score @s ball.own matches 1.. as @a[scores={ball.own=1}] at @s run tp @s ~ ~ ~ ~90 ~
            ...
            # IS NOT BALLOWNER, BALLOWNER doesn't EXIST => INVERT NEAREST PLAYER
      ...
      execute as @e[type=item,tag=!notCollected,tag=!returnItem,nbt={Item:{tag:{pjItem:["yes","booster"]}}}] at @s as @a[sort=nearest,limit=1] unless score @s player.posY matches 66.. unless data entity @s {ActiveEffects:[{Id:25b}]} at @s run fill ~1 ~ ~1 ~-1 ~5 ~-1 air replace cobweb
